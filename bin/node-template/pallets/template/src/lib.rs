...
decl_storage! {
    trait Store for Module<T: Trait> as TemplateModule {
        ...
        // cid -> [{owner, filename}]
        pub FsMap get(fn fs_map): map hasher(blake2_128_concat) Vec<u8> => (T::AccountId, Vec<u8>);
    }
}
...
decl_module! {
    ...
    pub struct Module<T: Trait> for enum Call where origin: T::Origin {
        ...
        pub fn submit_ipfs_results(origin, cid: Vec<u8>, name: Vec<u8>) {
			...
			// let data = IndexingData(b"submit_ipfs_results".to_vec(), cid.clone(), name.clone());
			// offchain_index::set(&key, &data.encode());
            // let data = IndexingData(who.clone().to_vec(), cid.clone(), name.clone());
            FsMap::<T>::insert(cid.clone(), (who.clone(), name.clone()));
                ...
                DataCommand::CatBytes(cid) => {
                    match Self::ipfs_request(IpfsRequest::CatBytes(cid.clone()), deadline) {
                        Ok(IpfsResponse::CatBytes(data)) => {
                            ...
                            let (_, filename) = FsMap::<T>::get(cid);
                            let signer = Signer::<T, T::AuthorityId>::all_accounts();
                            if !signer.can_sign() {
                                debug::error!("No local account available. Consider adding one via 'author_insertkey' RPC.");
                            }
